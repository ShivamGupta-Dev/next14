...
interface MagicButtonProps {
    ...
    position: string;
    handleClick?: () => void; // Define handleClick as a function that returns void
    otherClasses?: string;
}
...
const MagicButton = ({
    ...
    position,
    otherClasses,
    handleClick = () => {} // Provide a default empty function if handleClick is undefined
}: MagicButtonProps) => {
    return (
        <button className="relative inline-flex h-12 overflow-hidden rounded-lg p-[1px] focus:outline-none md:w-60 md:mt-10">
            ...
            <span
                ...
                onClick={handleClick} // Use handleClick directly as onClick event handler
            >
                {position === 'left' && icon}
                {title}
                {position === 'right' && icon}
